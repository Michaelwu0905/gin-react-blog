...
const EditPost = () => {
  ...
  if (error && !title) {
    return (
      <div style={{ padding: '20px', color: '#cc0000' }}>
        {error}
        <br />
        <button className="xp-button" onClick={() => navigate(-1)} style={{ marginTop: '10px' }}>
          返回
        </button>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {error && (
        <div style={{ background: '#ffdddd', color: '#cc0000', padding: '8px', marginBottom: '10px', fontSize: '12px' }}>
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ 
          ...
          <button type="button" className="xp-button" onClick={() => navigate(`/posts/${id}`)}>
            取消
          </button>
        </div>
      </form>
